...
resource "ibm_is_ssh_key" "generated_key" {
  name           = "${var.resource_prefix}-${var.region}-key"
...
resource "ibm_is_public_gateway" "zone" {
  ...
  name = "${var.resource_prefix}-${each.value.zone}"
  resource_group  = local.resource_group
  vpc             = ibm_is_vpc.location.id
  zone = each.value.zone
}
...
resource "ibm_is_subnet" "front" {
  ...
  name            = "${var.resource_prefix}-front-${each.key}"
  resource_group  = local.resource_group
  vpc             = ibm_is_vpc.location.id
  zone            = each.value.zone
...
resource "ibm_is_subnet" "back" {
  ...
  name            = "${var.resource_prefix}-back-${each.key}"
  resource_group  = local.resource_group
  vpc             = ibm_is_vpc.location.id
  zone            = each.value.zone
  ipv4_cidr_block = each.value.cidr
